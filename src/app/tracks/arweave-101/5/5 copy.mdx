...
<Callout emoji="💡">
  Note: Gateways come with GraphQL endpoints, but they can’t look into the body
  of a transaction; they can only access general transaction data and tags.
  Making tags the only way to customize your uploads for queries.
</Callout>
...
<Callout emoji="💡">
  Note: The names of these functions are a convention, so React Router can find
  them when importing the files later.
</Callout>
...
<Callout emoji="💡">
  Note: For development, you can also use the Vite dev server with `npm run
  dev`, but it’s always a good idea to check the code you will deploy from your
  `dist` directory.
</Callout>
...
<Callout emoji="💡">
  Note: My blog's blog owner address is hardcoded and used to fetch the list of
  articles. If you create or update an article, it won’t appear in the list.
</Callout>

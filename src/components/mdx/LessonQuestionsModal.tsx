...
export default function LessonQuestionsModal({
  ...
  return (
    <Dialog>
      <DialogTrigger
        className="font-poppins mx-auto my-3 flex w-fit rounded-3xl bg-[#721F79]
          p-2 px-4 text-2xl font-semibold text-white"
      >
        {buttonText}
      </DialogTrigger>
      <DialogOverlay />
      <DialogContent className="mb-32 mt-10 max-h-screen overflow-y-auto border-0 bg-[#1C1C1C] lg:min-w-[50%] ">
        <DialogHeader>
          <DialogTitle className="font-poppins m-2 text-2xl font-bold text-white">
          ...
          <DialogDescription
            className={`bg-[#1C1C1C] ${title === undefined ? "text-white" : ""}`}
          >
            {children}
          </DialogDescription>
        </DialogHeader>

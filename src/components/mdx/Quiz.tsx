...
const Quiz = (props: QuizProps): JSX.Element => {
  ...
  // const { refetchCompletedQuizzesAll } = useAppContext();
  ...
  // - Add
  const { mutate: quizzesAddMutate, isPending: quizzesAddIsPending } =
  ...
  const submit = () => {
    ...
    if (allLessonsData) {
      const lessonIdToSave: string = allLessonsData.find(
        ...
        (lesson: any) => lesson.quizFileName === props.quiz,
      )!.id;
  ...
  return (
    <Dialog>
      ...
      >
        <DialogHeader>
          <DialogTitle>
            <DialogTrigger className="w-full text-right text-[#44AF96]">
              X
            </DialogTrigger>
        ...
        <DialogFooter>
          <div className={`flex w-full items-center`}>
            <div className="flex w-full">
              ...
              >
                {!quizzesAddIsPending ? "Submit" : <Spinner />}
              </ButtonRaw>
            </div>
          </div>
        </DialogFooter>

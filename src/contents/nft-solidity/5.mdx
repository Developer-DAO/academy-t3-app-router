...
🚀 Creating a new RainbowKit app in ((path)):/d_d_academy/tierNFT/frontend
```
...
<Callout emoji="💡">
  ...
  `"@rainbow-me/rainbowkit": "^2.0.0",` to `"@rainbow-me/rainbowkit":
  "^1.0.0",`. This lesson is based on RainbowKit 1.x.
</Callout>
...
<Callout emoji="💡">
  Each file is pasted in this lesson, though if you want to view the full
  project, is is available at
...
<Callout emoji="💡">
  Here we start to use React Hooks, which are JavaScript functions that manage
  the state's behaviour and side effects by isolating them from a component.
</Callout>
